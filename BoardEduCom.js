...
const BoardEduCom = () => {
  ...
  const ComData = [
    { id: 1, title: '창업 스타터 패키지', writer: '관리자', date: '2023.11.12', screen: 'BoardEduComDetail' },
  ];
  ...
  const paginatedComData = ComData.slice(
    (currentPage - 1) * pageSize,
    currentPage * pageSize
    
  );

  const totalPages = Math.ceil(ComData.length / pageSize);

  const renderItem = ({ item }) => (
    <Card containerStyle={styles.card} key={item.id}>
      <TouchableOpacity onPress={() => navigation.navigate(item.screen, { ComId: item.id })}>
    ...
    <View style={styles.container}>
      <View style={styles.boardTitle}>
          ...
          {isLoggedIn && (userType === 'admin') && (
            <TouchableOpacity style={styles.button} onPress={() => navigation.navigate('BoardEduComWrite')}>
