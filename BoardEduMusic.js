...
const BoardEduMusic = () => {
  ...
  const paginatedMusicData = MusicData.slice(
    (currentPage - 1) * pageSize,
    currentPage * pageSize
    
  );

  const totalPages = Math.ceil(MusicData.length / pageSize);

  const renderItem = ({ item }) => (
    <Card containerStyle={styles.card} key={item.id}>
      <TouchableOpacity onPress={() => navigation.navigate(item.screen, { MusicId: item.id })}>
    ...
    <View style={styles.container}>
      <View style={styles.boardTitle}>
          ...
          {isLoggedIn && (userType === 'admin') && (
            <TouchableOpacity style={styles.button} onPress={() => navigation.navigate('BoardEduMusicWrite')}>
