...
const BoardEduCer = () => {
  ...
  const CerData = [
    { id: 1, title: '컴활 2급 자격증 과정 교육', writer: '관리자', date: '2023.11.12', screen: 'BoardEduCerDetail' },
  ];
  ...
  const paginatedCerData = CerData.slice(
    (currentPage - 1) * pageSize,
    currentPage * pageSize
    
  );

  const totalPages = Math.ceil(CerData.length / pageSize);

  const renderItem = ({ item }) => (
    <Card containerStyle={styles.card} key={item.id}>
      <TouchableOpacity onPress={() => navigation.navigate(item.screen, { CerId: item.id })}>
    ...
    <View style={styles.container}>
      <View style={styles.boardTitle}>
          ...
          {isLoggedIn && (userType === 'admin') && (
            <TouchableOpacity style={styles.button} onPress={() => navigation.navigate('BoardEduCerWrite')}>
