...
const BoardEduCul = () => {
  ...
  const CulData = [
    { id: 1, title: '기초 연극 교실', writer: '관리자', date: '2023.11.12', screen: 'BoardEduCulDetail' },
  ];
  ...
  const paginatedCulData = CulData.slice(
    (currentPage - 1) * pageSize,
    currentPage * pageSize
    
  );

  const totalPages = Math.ceil(CulData.length / pageSize);

  const renderItem = ({ item }) => (
    <Card containerStyle={styles.card} key={item.id}>
      <TouchableOpacity onPress={() => navigation.navigate(item.screen, { CulId: item.id })}>
    ...
    <View style={styles.container}>
      <View style={styles.boardTitle}>
          ...
          {isLoggedIn && (userType === 'admin') && (
            <TouchableOpacity style={styles.button} onPress={() => navigation.navigate('BoardEduCulWrite')}>
